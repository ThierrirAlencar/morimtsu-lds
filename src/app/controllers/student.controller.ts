...
import {
  CreateStudentDTO,
  promoteStudentDTO,
  promoteStudentRankDTO,
  QueryStudentFiltersDTO,
  UpdateStudentFormDTO,
  UpdateStudentPersonalDTO,
} from '../dto/student';
import z from 'zod';
...
export class StudentController {
  ...
  @ApiResponse({
    status: 409,
    description: 'Já existe um estudante com este email ou CPF.',
  })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  @ApiResponse({
    status: 405,
    description: 'Student is below 18 and needs to inform a parent contact',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'Student personal info updated successfully',
  })
  @ApiResponse({ status: 404, description: 'Student not found' })
  async updatePersonal(
    @Param('id') id: string,
    @Body() body: UpdateStudentPersonalDTO,
    @Res() res: Response,
  ) {
    try {
  ...
  @ApiResponse({
    status: 200,
    description: 'Student form updated successfully',
  })
  @ApiResponse({ status: 404, description: 'Student not found' })
  async updateForm(
    @Param('id') id: string,
    @Body() body: UpdateStudentFormDTO,
    @Res() res: Response,
  ) {
            ...
            
{
  ...
  @ApiResponse({
    status: 200,
    description: 'Student joined class successfully',
  })
  @ApiResponse({ status: 404, description: 'Student or class not found' })
  @ApiResponse({
    status: 405,
    description: 'Student is too young or old to join this class',
  })
  async joinClass(
    @Param('id') id: string,
  ...
  @ApiResponse({
    status: 404,
    description:
      'Student or class not found or student is not even assigned to this class',
  })
  @ApiResponse({
    status: 405,
    description: 'Student is too young or old to join this class',
...

{
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Search in name, email or nickname',
  })
  @ApiQuery({
    name: 'minAge',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: String,
    description: 'Filter by exact CPF match',
  })
  @ApiQuery({
    name: 'email',
    required: false,
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Filter by student form presence',
  })
  @ApiQuery({
    name: 'Rank',
    required: false,
    enum: Rank,
    description: 'Filter by student rank',
  })
  @ApiQuery({
    name: 'class',
    required: false,
  ...
  async query(
    @Query() queryParams: QueryStudentFiltersDTO,
    @Res() res: Response,
  ) {
    try {
      const filters: QueryStudentFiltersDTO = {
        ...
        Presence: queryParams.Presence
          ? Number(queryParams.Presence)
          : undefined,
  ...
  @ApiParam({
    name: 'studentId',
    description: 'Id do estudante a ser promovido',
  })
  @ApiResponse({ status: 200, description: 'promovido com sucesso' })
  @ApiResponse({
    status: 401,
    description:
      'Ação não autorizada, observe o log da resposta para mais detalhes',
  })
  ...
  async promote(
    ...
    try {
      const _service = await this.studentService.promoteStudentToCoach(
        studentId,
        password,
        id,
      );
  ...
  @ApiParam({
    name: 'studentId',
    description: 'Id do estudante a ser promovido de rank',
  })
